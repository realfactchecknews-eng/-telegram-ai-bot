...
В файле `bot.py` ищи три блока:

### Системный промпт (строки 24–33)
...
Здесь вставляются конкретные фразы. Чем больше примеров — тем точнее стиль. Добавляй пары вопрос-ответ в этом формате.

### Случайные вставки (строки 74–82)
...
Каждый ответ начинается с одной из этих фраз. Можешь менять или добавлять.

## Как бот запоминает разговор

Бот хранит последние 10 сообщений для каждого пользователя. Это позволяет вести связный диалог. История хранится в памяти, поэтому после перезапуска бота она сбрасывается.
