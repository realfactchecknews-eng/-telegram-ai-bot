...
Простой бот в Telegram, который отвечает на вопросы через ИИ. Поддерживает несколько API: Groq, OpenRouter, Together, HuggingFace, Cerebras. Если один не работает, бот пробует следующий. Бот запоминает контекст разговора и использует стиль с мемными фразами.
...
В `bot.py` можно менять:

- `SYSTEM_PROMPT` — общий стиль и тон бота.
- `FEW_SHOT_EXAMPLES` — конкретные примеры вопросов и ответов. Чем больше, тем точнее стиль.
- `CATCHPHRASES` — фразы, с которых бот начинает ответы.

### История диалога

Бот хранит последние 10 сообщений для каждого пользователя, чтобы вести связный разговор.
